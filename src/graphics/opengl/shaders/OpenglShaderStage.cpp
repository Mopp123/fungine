    ...
    namespace graphics
    {
        namespace opengl
        {
            ...
            unsigned int OpenglShaderStage::loadFromSource(const std::string& path, ShaderStageType type)
            {
                ...
                FILE* file = nullptr;
                // NOTE: just a quick fix to get running on linux..
                #ifdef _WIN32
                ...
                // NOTE: just a quick fix to get running on linux..
                #ifdef _WIN32
                readBytes = fread_s(shaderSource, sizeof(char) * fileLength, sizeof(char), fileLength, file);
                #elif __linux__
                readBytes = fread(shaderSource, sizeof(char), fileLength, file);
                #else
                // error...
                #endif
